        ...
        <div class="nxl-content">
            ...
            <div class="container pt-5">
                @if (isset($errors))
                    ...
                    @endforeach
                @endif
                <form action="/admin/makaleler/add" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{$data->id ?? ''}}">
                    <div class="formgroup">
                        <label for="title">Başlık</label>
                        <input type="text" class="form-control" placeholder="Makale Başlığı" name="title" value="{{ old('title', isset($data) && isset($data->title) ? $data->title : '') }}">
                    </div>
                    <div class="formgroup mt-4">
                        <label for="tags">Etiketler</label>
                        <input type="text" class="form-control" placeholder="etiket1,etiket2 ..." name="tags" value="{{ old('tags', isset($data)  && isset($data->tags) ? $data->tags : '') }}">
                    </div>
                    <div class="formgroup mt-4">
